...
st.set_page_config(page_title="Atelier OJ", page_icon="⌁", layout="wide")
...
:root { --paper:#f3f0e8; --ink:#102a43; --muted:#637282; --acid:#d9ff52; --line:#c8c8bd; }
.stApp { background:
  linear-gradient(rgba(16,42,67,.035) 1px,transparent 1px),
  linear-gradient(90deg,rgba(16,42,67,.035) 1px,transparent 1px),var(--paper);
...
[data-testid="stSidebar"] .stButton button { border-color:#637282; background:transparent; }
h1,h2,h3 { color:var(--ink); letter-spacing:-.035em; }
h1 { font-size:3.3rem !important; line-height:1 !important; }
code,.mono,.eyebrow,[data-testid="stMetricLabel"] { font-family:'DM Mono',monospace !important; }
.eyebrow { text-transform:uppercase;letter-spacing:.13em;font-size:.72rem;color:var(--muted); }
.mast { border-top:5px solid var(--ink);padding-top:1rem;margin-bottom:2rem; }
.signal { display:inline-block;background:var(--acid);color:var(--ink);padding:.22rem .55rem;
  border-radius:2px;font-family:'DM Mono',monospace;font-size:.72rem;font-weight:500; }
.plate { border:1px solid var(--line);background:rgba(255,255,255,.46);padding:1.25rem;
  box-shadow:5px 5px 0 rgba(16,42,67,.08); }
.stButton button { border-radius:2px;border:1px solid var(--ink);font-weight:600; }
.stButton button[kind="primary"] { background:var(--ink);color:white; }
[data-baseweb="input"]>div,[data-baseweb="select"]>div,textarea { border-radius:2px !important; }
@media(max-width:760px){
  h1{font-size:2.35rem!important}.block-container{padding:1.5rem 1rem!important;}
}
</style>
"""
...
def call(action: object) -> dict[str, object] | None:
    try:
...
def heading(kicker: str, title: str, note: str = "") -> None:
    ...
    st.markdown(f'<div class="mast"><h1>{title}</h1></div>', unsafe_allow_html=True)
    if note:
        st.caption(note)
...
def problem_form(api: ApiClient, existing: dict[str, object] | None = None) -> None:
    ...
    with st.form("problem-editor"):
        ...
        submitted = st.form_submit_button(
            "保存修改" if existing else "创建题目", type="primary", use_container_width=True
        )
        if submitted:
            try:
                body = {
...
def submit_page(api: ApiClient) -> None:
    ...
    if st.button("提交给评测器", type="primary", use_container_width=True):
        result = call(
            lambda: api.post(
                "/api/submissions/",
...
def live_submission(api: ApiClient, submission_id: str) -> None:
    ...
    if logs:
        st.dataframe(logs["data"]["details"], use_container_width=True, hide_index=True)
...
def admin_page(api: ApiClient) -> None:
    ...
    with users_tab:
        ...
        if users:
            st.dataframe(users["data"]["users"], use_container_width=True, hide_index=True)
            mapping = {
    ...
    with audit_tab:
        ...
        if logs:
            st.dataframe(logs["data"], use_container_width=True, hide_index=True)
    with visibility_tab:
...
def ai_page(api: ApiClient) -> None:
    ...
    with task_tab:
        ...
        if st.button("开始智能命题", type="primary", use_container_width=True):
            result = call(
                lambda: api.post(
...
def auth_screen(api: ApiClient) -> None:
    ...
    with right:
        ...
        with login_tab, st.form("login"):
            ...
            if st.form_submit_button("进入工作台", type="primary", use_container_width=True):
                result = call(
                    lambda: api.post(
