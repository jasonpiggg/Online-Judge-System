...
题目未公开日志时，提交者可以查看总得分和总分但不能读取逐点明细，其他普通用户返回 403；题目公开后已登录用户可以查看逐点日志。管理员始终可以查看完整逐点日志。日志公开仍不会开放私有提交概要。存在且有效的日志访问分别记录 200/403；未登录、非法参数或不存在的记录不写审计。API 默认不返回内部异常、密钥或服务端临时路径。
