...
日志响应始终包含 `score/counts`。题目未公开日志时，提交者本人可以访问日志，但不会收到
`details`；其他普通用户返回 403。题目公开日志后，已登录普通用户会额外收到逐点状态、耗时和
内存组成的 `details`。管理员不受公开设置影响。日志接口不会返回源码、隐藏输入、标准输出或
可能带源码上下文的编译日志；这些提交详情仍只允许提交者本人和管理员访问。成功和拒绝访问
继续分别写入 200/403 访问审计。
