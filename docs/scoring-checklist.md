...
课程得分点在默认 React 网页中的逐项入口见
[实验功能与 React 网页覆盖核对](web-scoring-coverage.md)。
...
| Advance R1–R4     |    4 | 可读命题工作台、自定义兼容模型/密钥/单价、流式阶段状态、真实取消、Token 与费用                              | `ai/authoring.py`、`ai/transport.py`、`frontend/ai.py`                            | AI HTTP/SSE mock、浏览器生成与取消测试                                                                  |
| AI 质量与易用性   |    6 | 二次批判、独立 oracle 与受限生成器对拍、mutation score、版本草稿、发布门禁、编辑器局部入口                  | `ai/experience.py`、`routers/authoring.py`、`frontend/ai.py`、`frontend/editor.py` | mock 自动测试验证执行链；真实外部供应商仍需单独付费验收                                                |
