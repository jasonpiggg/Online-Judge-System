...
| Step 6 Web UI | 5 | 账户、题库搜索筛选分页、同屏做题、手机三标签、会话草稿、分区编辑器、提交详情、管理中心 | `frontend/` | `test_frontend.py`、`test_frontend_workflows.py`、`test_frontend_release.py`；1440/1024/390 真实浏览器 |
...
| AI 质量与易用性 | 6 | 生成后第二次批判改进、至少 5 个唯一输入、基础/边界/规模审查、参考解全过、两种错误解被有效测试点拒绝、标准编辑器新增/确认更新 | `ai_authoring.py`、`frontend/ai.py`、`frontend/editor.py` | 真实 HTTP mock 自动测试；浏览器新增和更新入库。真实外部供应商未验收 |
...
- AI：`GET/PUT /api/ai/model-config`、`POST /api/ai/problem-tasks/`、`GET /api/ai/problem-tasks/{task_id}`、`PUT .../cancel`。
