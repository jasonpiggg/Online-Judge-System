...
Streamlit 生成整题使用 `basic_draft`：基础内容生成、参考解预检、轻量复审、最终基础验证。
只需完整题面、样例、Python 参考解及至少五个不同测试输入；不要求高级验证资产。
系统使用 Flash high 构思生成，GLM-5.3 low 复审；复审未配置时使用当前模型，个人配置优先。
每阶段最多8192输出 Token（含推理），共享含排队的240秒预算，最后5秒留给保存与清理。
复审只阻塞未解决的正确性问题，排版和扩充覆盖建议不阻塞；参考解必须通过全部样例与测试。
最多一次额外定向修复；网络错误、服务商错误及截断不自动收费重试。
成功表示基础验证通过，完整验证未执行；不会自动发布，也不标记完整质量门禁通过。
API 默认 `full` 与显式 `balanced` 保留兼容；高级资产通过独立“补全验证资产并完整验证”任务处理。
AI 编辑不设最少字符数，空需求按所选范围自动优化；新题无参考内容时要求非空需求。
关闭和清空标签立即执行，不取消后台任务或删除草稿；未保存内容仍使用既有本地备份机制。
...
- 工作标签保留标签内返回历史，普通入口优先恢复已打开的同一页面。所有应用内跳转使用当前页，不再提供额外的新标签页菜单；已打开的目标会直接恢复。回到题库等首页时，工作标签保留并取消选中。
- “编辑题目”复用当前用户的未归档、未发布草稿；新接口 `POST /api/problems/{problem_id}/editing-draft` 在事务中查询或创建，返回现有草稿结构。主动创建独立草稿仍使用原接口。
- 命题中心支持导入单道题目的 JSON 对象（UTF-8，可含 BOM，上限 10 MiB），并提供可下载示例。可先导入未完成的字段，再补全和验证。未知字段由服务端拒绝；题号已存在时须修改后重新导入。文件不能携带审核或发布状态。
- 做题页“导入代码文件”先预览，确认替换目标语言草稿后再手动提交。上限 1 MiB 且不超过 200,000 字符，仅接受 UTF-8 文本，保留换行和缩进，不支持 ZIP 工程。
- 支持扩展名来自 `/api/languages/?include_metadata=true` 的 `file_ext`，初始为 `.py`、`.cpp`。注册新语言后自动刷新支持列表，不需重新构建前端；服务端须预先具备对应编译器或运行环境。扩展名共用时可选择语言，未提供语法高亮的语言使用纯文本编辑。
- 代码字体使用随站点分发的 JetBrains Mono Regular，资源与 OFL-1.1 许可证位于 `web/public/fonts/`，运行时无需访问外部字体服务。


### 草稿 JSON 与 Windows C++ 评测

- 在草稿的「检查与发布 → 高级：JSON 导入与导出」中，点击「下载 JSON」导出当前表单（包含未保存修改）；也可展开预览后复制。
- 「导入本地 JSON」直接选择 UTF-8 的 `.json` 文件（上限 10 MiB），无需粘贴到文本框；也保留粘贴后点击「载入 JSON」的方式。导入经字段校验后替换当前表单，点击「保存草稿」同步；不会直接发布题目。导出内容仅含题目字段，不包含参考解、暴力解或生成器等独立验证资产。
- Windows C++ 评测为每次任务提供可写的独立临时目录，并优先从实际编译器目录加载 MinGW 运行时 DLL，避免 Anaconda 等环境中同名 DLL 干扰。无需修改系统 PATH；仍须安装可用的 `g++`。


### 日志可见性与界面

| 日志设置 | 提交者本人 | 其他已登录普通用户 | 管理员 |
| --- | --- | --- | --- |
| `public_cases=False` | `score`、`counts`，无 `details` | 403 | 完整测试点日志 |
| `public_cases=True` | 得分及完整测试点日志 | 得分及完整测试点日志 | 完整测试点日志 |
