...
export function ActivityProvider({ userId, children }: { userId: string; children: ReactNode }) {
  ...
  useEffect(() => {
    try { localStorage.setItem(storageKey, JSON.stringify(entries.slice(0, 20))); }
    ...
    <div className="activity-strip" aria-label="进行中的任务">
      <div className="activity-strip-inner">
        ...
        <div className="activity-tabs">
          {visible.map((entry) => (
            <div className={`activity-tab ${location.pathname + location.search === entry.path ? "active" : ""}`} key={entry.id}>
          ...
          {overflow.length > 0 && (
            <details className="activity-more">
              ...
              <div>{overflow.map((entry) => <div className="activity-overflow-row" key={entry.id}><Link to={entry.path}>{entry.title}<small>{entry.status}</small></Link><button type="button" aria-label={`关闭 ${entry.title}`} onClick={() => close(entry)}><Icon name="close" /></button></div>)}</div>
            </details>
          )}
        </div>
      </div>
